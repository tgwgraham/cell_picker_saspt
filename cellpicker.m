function cellpicker(basefname,range,outfile,gridsize,selectedByDefault)
...
while m < nmontages
    ...
    for j=n:min(n+nrows*ncols-1, range(2))
        ...
        if isempty(masks{j})
            seg = csvread(segf);
            if selectedByDefault == 1
                seg = -seg;
            elseif selectedByDefault == 2
                % select only the largest ROI
                largestCellIndex = mode(seg(seg(:) ~= 0));
                seg(seg==largestCellIndex) = -seg(seg==largestCellIndex);
            end
            masks{j} = seg;
        else
            seg = masks{j};
